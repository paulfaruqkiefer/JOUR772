...
**Answer** https://datawrapper.dwcdn.net/V37IH/1/
...
**Task** Write code to add columns showing the total number of votes for each county and the percentage of votes received by Wes Moore in each county (you did this in Lab 8(?)), then replace the CSV file in the data folder with it.
...
md_gov_county <- md_gov_county |>
  ...
  mutate(moore_voteshare = moore/total*100) |> 
  arrange(desc(moore_voteshare))
...
Since I started with the Maryland-specific map, this step was redundant - I have data for every Maryland jurisdiction.
...
When I hover over a jurisdiction, the map now displays the name of the jurisdiction in bold letters and Moore's vote share - as a percentage - below. 
...
https://datawrapper.dwcdn.net/btdyC/1/
