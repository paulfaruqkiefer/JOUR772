...
census_api_key("8efce88f8d41d361a0ab0045b4131f78ee063fb6", install=TRUE)
```
...
Nevada grew the fastest - with a 35 percent increase - and Texas saw the largest growth by total number of new residents. 
...
Howard County has the highest median household income in Maryland at an estimated $129,549 with a margin of error of $2980. 

Somerset County has the lowest median household income in Maryland at an estimated $48,661 with a margin of error of $4381. 
```{r}
...
md <- md |> arrange(desc(estimate)) |> top_n(24, estimate)
...
The headline should read: "Howard County is almost certainly the wealthiest county; Calvert and Montgomery are fighting for second place."
...
The margin of error for Calvert County is large enough to make the difference between Calvert and Montgomery Counties insignificant. 

Is the difference between Howard and everyone else significant? **Answer**

Howard County has a small enough margin of error that the estimated household income for Howard County does not overlap with any other county, so it is almost certainly wealthier per capita than any other county in Maryland. 
...
Baltimore City saw the largest increase in median household incomes -- driven by an influx of wealthy residents. Allegany and Calvert Counties also saw median household incomes grow by more than 30 percent. No counties saw median household incomes fall, though Caroline County saw by far the lowest income growth in the state.
...
We could ask whether the zip codes with the highest enrollment percentages correspond to the zip codes with the highest median household incomes. 
...
The median household income in zip codes with high schools that have enrollment rates over 60 percent is 1.6 times higher than the median household income in zip codes with high schools that have enrollment rates below 40 percent. 
...
Which county has the largest disparity in median household incomes between zip codes containing high schools?
