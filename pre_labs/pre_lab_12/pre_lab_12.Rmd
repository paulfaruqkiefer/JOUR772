...
The date column is a character, and the "Press Release" column does not have an ideal column name.
...
What time of day were the press releases issued?

Would it be possible to extract a subject matter (categorical) from the web page?
...
The output suggests there are far more <ul> tags in the HTML source than we need; we will need to filter for <li> tags within the <ul> tags to drop the extraneous tags.
...
The output lists the tags themselves without the HTML text, which gives us a clearer sense of which tags we are after.
...
The output is a single column containing all the tags; for press releases, the tags contain both the date of the release and the title of the release separated by a colon. 
