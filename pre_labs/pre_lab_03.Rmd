...
The red warning line instructs the user to use the problems() function to identify parsing issues -- i.e. when determining column types.
...
Because the guess_max is now equal to the length of the dataset, the codeblock no longer shows a parsing issue because the additional guesses enabled the program to detect that Zip Codes should be treated as a character field rather than a numerical field.
...
The "amount" field is now a <dbl> data type, meaning a number. The "date" column is now a <chr>, or character, type.
...
Thousands of zip codes do not have five characters. In some cases, those are the result of foreign postal codes entered into the zip code field. Others could simply be entry errors.
...
The clean_names() function replaced all column names that previously contained spaces with column names that use underscores in place of spaces. The function also converts all column names to lower-case. 
...
The new dataframe has 17,740 rows -- 25 fewer rows than the previous dataframe.
```{r}
# cleaning function
cleaned_md_grants_loans <- md_grants_loans |>
  clean_names() |> 
...
The function converts any nine-digit zip codes to five-digit zip codes, though many zip codes in the previous dataframe were already in five-digit format.
...
Zip code 21202 in downtown Baltimore received the largest sum from the Maryland tourism board, possibly because the state hopes to revive downtown Baltimore by drawing additional tourism revenue to the neighborhood. 


```{r}
maryland_tourism_board <- cleaned_md_grants_loans |> filter(source == 
"COMMERCE/MARYLAND TOURISM BOARD")

maryland_tourism_board
```
