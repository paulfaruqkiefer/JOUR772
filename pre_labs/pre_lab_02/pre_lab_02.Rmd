...
For character columns, the summary() function provides the number of values in the column (i.e. length) and the data type (character). For numerical columns, the summary() function provides basic exploratory analytics of the data within a column, including minimum, maximum, mean, median and the first and third quartile. 

The colnames() function only provides the name/title for each column in a dataset.
...
Creating a new variable to store the result enables us to manipulate or analyze a portion of a dataset (or a cleaned-up version of the dataset) without overwriting the original dataset.
...
The OR operator selects all observations/values that meet any of the specifications, but not necessarily all of the specifications. In this case, it not only includes all Theater courses, but also all courses with 15 or more seats.
...
The mutate() function transformed the month/day/year format of the original dataset to the standard year-month-day format, which R recognizes as a date rather than a string of characters.
...
The month column groups all expenses/spending by month (within 2022), using the first day of the month as a stand-in to represent all days in a given month. 
...
**Answer**
This code defines the variable total_votes as the sum of all votes for each candidate (and write-ins). It then defines a second variable, pct_moore, as the number of votes for Wes Moore divided by the previous variable total_votes. The mutate() function then appends those columns to the original dataframe.
...
The previous task ordered the data alphabetically by county in ascending order, whereas this code orders the data by pct_moore in descending order, using percentages rather than decimals.

**I do not know why the dataset was ordered alphabetically by county by default**
...
This answer is nominally more useful for the purposes of comparing Moore's lowest county-level margins to those of Democratic gubernatorial candidates in states with similar partisan vote shares, but it is not generally more useful than the previous answer. 
...
The tidyverse function str_to_lower changes all characters to lowercase format. The function str_to_title converts to title case, meaning only the first character of each word is capitalized (in srings containing multiple words). 

**Does R distinguish between words in a string by noting blank spaces?**
...
This function searches the "address" column for a pattern. In this code, those patterns are the full names of states, which the code associates with the abbreviated name of each state, i.e. Virginia = VA. When the code detects a pattern in the address field, it adds the corresponding abbreviated state name in the "state" column. 

**Does this code double-count West Virginia addresses?**
**Also, this counts Washington, DC as Washington State**
...
Again, there is some possibility that Virginia is overcounted (because of possible double-counting of West Virginia expenses), but Washington State is clearly overcounted because the code categorizes Washington, DC addresses as "WA" in the "state" column. 

Otherwise, MD as the primary location of expenses is clearly reasonable, and a sizeable portion of the WA expenses are Amazon purchases (Amazon is headquartered in Seattle).
...
1. Which courses have the highest/lowest number of seats relative to their departments' median?

2. Which states have the highest median expense per transaction?

3. In which Maryland zip codes did campaign committees spend the most?
