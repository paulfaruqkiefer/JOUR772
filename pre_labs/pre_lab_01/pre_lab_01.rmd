...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer** The "id" column is the <chr> data type and the "seats" column is the <dbl> data type.
...
**Answer** The code is identifying the courses in each term -- listed as a six-digit number in the UMD registry -- and grouping those courses. It then counts the number of classes (or tokens?) in each group.
...
The second codeblock rearranges the groups (terms) by the number of courses offered per term in descending order. The previous codeblock arranged the groups (terms) chronologically.
...
**Answer** This codeblock groups courses by (1) term and (2) department. It then lists the number of courses listed in each term by each department, arranged in order of term number. 
...
This codeblock groups courses by term and calculates the total number of seats available in all classes offered in that term. It then arranges the terms in descending order by the number of seats available in that term.
...
Like the previous codeblock, this codeblock groups courses by term and calculates the total number of seats available in all classes offered in that term. Unlike the previous codeblock, it then calculates the mean and median number of seats per class in a given term. The codeblock then lists the groups in descending order by the total number of seats available in a given group (term).

The top terms are all fall terms.
...
Like the previous codeblock, this codeblock groups courses by term, calculates the total number of seats available in all classes offered in that term, and calculates the mean and median number of seats per class in a given term. Unlike the previous codeblock, it then calculates the minimum and maximum number of seats offered in any class in a term. The codeblock then lists the groups in descending order by the total number of seats available in a given group (term)
...
1.  Which courses (id) are offered in the most terms?

2.  Which department's courses have the highest mean and median number of seats?

3.  Which department has the highest mean syllabus count?
  
**Admittedly, I do not know what a syllabus count of 87 (for example) means**
