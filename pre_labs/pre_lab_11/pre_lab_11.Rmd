...
The output is a dataframe of press releases from Cardin's office with a column for release dates, a column for release titles, a column for release urls, and a column for the release texts.
...
The unique() function identifies all unique strings in a list.
...
This code creates a dataset of unique words in Cardin's press releases, with a single cell for each word. 
...
This code returns a dataset of words in Cardin's press releases that do not appear in the stop_words dataset. It provides a more revealing summary of Cardin's rhetorical priorities -- namely his focus on health care policy, it seems.
...
This creates a unique list of words in press releases by year using an anti-join with the stop_words dictionary.
...
There is an odd bigram that should clearly be three-word phrases -- "19 pandemic" -- but the standouts are apparent references to the "American Rescue" Plan Act (half of a four-word phrase), the Chesapeake Bay and health care/public health.
...
Cardin's press releases in 2022 appear to be more project funding-oriented, and he apparently made significantly more references to Maryland jurisdictions by name.
...
A few standout bigrams appear in 2023, including "oral health" -- a reflection of Cardin's advocacy for expanding medicaid to cover dental care and industry recognition for those efforts -- as well as "African American," "Supreme Court," "offshore wind" and "tax credit." Those point to cultural debates, policy considerations and appointment decisions that were not at the top of Cardin's messaging priority list in earlier years. 
...
I am an avid rail transportation fan, so I am a bit perturbed that the sentiment function reads "rail" as a negative. I'm also not convinced that "immense" is, on balance, a positive word. 
