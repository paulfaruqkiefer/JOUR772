...
ZIPCODE2 -- A 5-digit zip code with the prefix "024001," which is used to identify the zip code with the corresponding state (Maryland)
ZIPNAME -- A named geographic location in the zip code
ZIPCODE1 -- The 5-digit zip code.

It also includes geometric information, including Shape_Leng, Shape_Area, and geometry, which are used to define boundaries of zip codes on a map. 
...
The output is a standard dataframe with the columns listed above; the geometric data is treated as columns. 
...
The output is a blank map of Maryland zip codes, with higher densities of zip codes in more populous areas. 
...
This code loads the tigris library, which enables us to load shapefiles directly from the US Census Bureau.
...
The output is a list of column descriptions; all but three are <chr> type, including the state FIPS codes and county names. There are two <dbl> columns and one <MULTIPOLYGON> column. The latter contains the shapes of every county in the country. 
...
The output is made up of all counties with the FIPS code 24, which represents Maryland. In other words, the outcome is a filtered dataset of Maryland counties. 
...
The output is a blank map of Maryland counties, but the county boundaries include the portions of the Chesapeake Bay included in each county, obscuring the difference between land and water. 
...
The output is a map of Maryland zip codes overlaid over a map of Maryland counties. 
...
The output is a dataframe of Maryland foreclosure notices by zip code. It includes some non-Maryland zip codes,
...
The output is a dataframe that joins the data associated with zip codes in the Maryland foreclosure notice dataset with the corresponding zip codes in the Maryland zip code map. This filters out non-Maryland zip codes. 
...
This code outputs a map of Maryland zip codes shaded to reflect the number of foreclosure notices issued in said zip code in March 2023.
...
This loads the tidycensus package (among others) to allow us to include census data in our analysis.
...
The output has two elements: A new dataset of foreclosure notices by county and a dataset of county populations in the 2021 ACS. The foreclosure notice by county dataset is altered to turn "county" columns into rows.
...
This uses a feature of tidycensus that includes the geometry associated with geographic demographic information. It returns a dataframe of county populations that includes the geometry for each county.
...
We have two dataframes -- one for county population and one for foreclosure notices by county. We need to join by the county names, but they are not in a matching format. 
...
This code renames the county column in the md_county_population to "NAME," converts county names to title case, removes extra blank space on both sides of the county name, renames the demographic column from "estimate" to "population", and displays the county, population and geometry columns in that order.
...
This code joins the foreclosure notice dataset with the county population dataset by county name. 
...
This code returns a dataframe with the per capita rate of foreclosure notices by county. 
...
This code returns a map of Maryland counties with levels of shading corresponding to per capita foreclosure notice rates. It uses the county centroids dataframe to identify county boundaries. 
...
This codes uses the scale_fill_viridis_b function to assign a color to a county corresponding to the range in which the county's foreclosure notice per capita rate falls. 
...
This uses a logarithmic scale to adjust the foreclosure per capita ranges used to assign colors to counties. 
